    ...
    def __getitem__(self, index):
        ...
        if label.shape[0] != self.crop_d or label.shape[1] != self.crop_h or label.shape[2] != self.crop_w:
            ...
            label = zoom(label,
                         ...
                         order=0)
            # 避免缩放时小目标标签丢失。
            label = np.zeros(label.shape, dtype=np.uint8)
            label[label>0] = 1
            label[label>127] = 2
            print(datafiles["name"],Counter(label.flatten()))
            print(datafiles["name"],Counter(label.flatten()))

        if not os.path.exists('./cases/show'): os.makedirs('./cases/show')
        ...
        sitk.WriteImage(save_image, './cases/show/image_%s.nrrd'%datafiles['name'])
        ...
        sitk.WriteImage(save_label, './cases/show/label_%s.nrrd' % datafiles['name'])
